...
The generated set currently includes 42 samples covering passing applications, readable critical mismatches, missing labels, low-quality OCR review cases, Formula ID support, TTB ID Number formula references, exact Formula ID matching, no-formula-required applications, final-alcohol ranges, matched formula documents without final alcohol content, proof/ABV normalization including low-proof formula support, `Proof 90`, `Alc. 45% by Vol.`, and `Alcohol 45% by Volume` wording, non-alcohol percentages such as `100% Agave`, net contents normalization, serving-size volume false-positive prevention, imported-country pass/review cases, missing required application values, supplied fanciful-name review, brand text appearing only in bottler/importer context, class/type text appearing only inside the brand line, bottler/producer text appearing only outside responsible-party context, combined responsible-party wording such as `Produced and bottled by`, product-type mismatch failures, product-type descriptors such as wine-cask-finished spirits, Item 15 review, and color-artwork labels. See `samples/expected_outcomes.md` for the full sample list and expected Pass / Needs Review / Fail outcomes.
