...
- Product type materially mismatched. Explicit `DISTILLED SPIRITS`, `MALT BEVERAGE(S)`, and wine statements in product/class label context are prioritized over incidental descriptors or brand words such as `Wine Cask Finish` or `Spirit` inside a wine brand, even when the explicit product-type statement appears before the brand.
