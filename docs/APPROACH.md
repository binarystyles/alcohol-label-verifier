...
- Imported country of origin is missing or the country name appears only in unrelated text, such as an importer company name, rather than in origin wording like `Product of`, `Produce of`, `Distilled in`, `Brewed in`, `Made in`, `Imported from`, or `Imported by ... from`. United States and United Kingdom origins can match common label abbreviations such as `USA`, `U.S.A.`, `U.S.`, `UK`, or `U.K.`; United Kingdom can also match constituent-origin wording such as `Product of Scotland` or `Distilled in Scotland`; Ireland can match `Republic of Ireland`; and Netherlands can match `The Netherlands`.
