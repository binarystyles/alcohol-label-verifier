...
- Required brand materially mismatched. Brand matching is limited to primary label text and preserves word order. Harmless case, apostrophe, punctuation, and ampersand/`AND` variations pass, but a brand name appearing only in producer, importer, warning, or other non-brand context, or with reordered brand words, does not create a false pass.
