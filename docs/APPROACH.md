...
- Product type materially mismatched. Explicit `DISTILLED SPIRITS`, `MALT BEVERAGE(S)`, beer-style terms, `MALT LIQUOR`, and wine statements in product/class label context are prioritized over incidental descriptors or brand words such as `Wine Cask Finish` or `Spirit` inside a wine brand, even when the explicit product-type statement appears before the brand.
- Material ABV mismatch. Proof values from labels and formula support are converted to ABV, including low-proof values such as `40 Proof` = `20% ABV` and label wording such as `Proof 90`. Common label wording such as `Alc. 45% by Vol.`, `45% Alc. by Vol.`, and `Alcohol 45% by Volume` is normalized to ABV. Non-alcohol percentages such as `100% Agave` are ignored for ABV/proof matching and become Needs Review if no actual alcohol statement is found.
- Material net contents mismatch. Milliliters, liters, centiliters, fluid ounces, plain ounces used as net contents, pints, and compound pint/ounce statements are normalized to milliliters. Serving-size, recipe, calorie, or per-serving volumes are ignored so they do not create false net-contents mismatches.
...
- Bottler/producer text appears only in brand or other non-responsible-party context instead of wording such as `Bottled by`, `Distilled by`, `Imported by`, or `Produced and bottled by`.
...
- Imported country of origin is missing or the country name appears only in unrelated text, such as an importer company name, rather than in origin wording like `Product of`, `Produce of`, `Made in`, or `Imported from`. United States origin can match common label abbreviations such as `USA`, `U.S.A.`, and `U.S.`.
