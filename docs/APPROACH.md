...
- Government warning missing, materially altered, or shown with a readable non-all-caps or noncanonical punctuation heading on a readable label.
...
- Product type materially mismatched. Explicit `DISTILLED SPIRITS`, distilled-spirits class/type terms such as `Gin`, `Liqueur`, `Whiskey`, `Distilled Spirits Specialty`, `Vodka Cocktail`, or tightly recognized first-line class phrases such as `STRAIGHT RYE WHISKEY`, `MALT BEVERAGE(S)`, `FLAVORED MALT BEVERAGE`, beer-style terms such as `Beer`, `Lager Beer`, `India Pale Ale`, `IPA`, `Pilsner`, `Gose`, `Witbier`, `Hard Seltzer`, or `Spiked Seltzer`, modified first-line beer/seltzer phrases such as `SPARKLING HARD SELTZER` and `IMPERIAL STOUT`, `MALT LIQUOR`, hard-cider/perry terms, CFR wine class/type terms such as `Sake`, `Vermouth`, `Sherry`, `Port`, `Champagne`, `Mead`, and `Sangria`, common grape-varietal type designations such as `Chardonnay` or `Cabernet Sauvignon`, and wine statements in product/class label context are prioritized over incidental descriptors or brand words such as `Wine Cask Finish`, `Wine Barrel Finished`, `Made with wine botanicals`, or `Spirit` inside a wine brand, even when the explicit product-type or `Class/Type` statement appears before the brand.
- Material ABV mismatch. Proof values from labels, expected application data, and formula support are converted to ABV, including low-proof values such as `40 Proof` = `20% ABV`, expected proof ranges such as `80-90 Proof` or `80 Proof - 90 Proof`, and label wording such as `Proof 90` or `90 degrees proof`. Common label wording such as `Alc. 45% by Vol.`, `45% Alc. by Vol.`, `45% by volume`, `Alcohol 45% by Volume`, `Alcohol: 45% by Volume`, `45% Alcohol Vol.`, `45% Alcohol/Vol.`, `13.5% vol`, repeated-unit ranges such as `12.5%-13.5% ABV`, decimal-comma `13,5% vol`, decimal-comma ranges such as `12,5-13,5% ABV`, and `Alcohol 45% by Vol.` is normalized to ABV. Non-alcohol percentages such as `100% Agave`, ingredient percent-by-volume text such as `Natural flavoring 49% by volume`, and non-ABV volume text such as `Alcohol Volume 50 mL` are ignored for ABV/proof matching and become Needs Review if no actual alcohol statement is found.
- Material net contents mismatch. Milliliters, litres, British `millilitres`, decimal-comma litres such as `0,75 L`, centiliters, fluid ounces, plain ounces used as net contents, dotted or OCR-split metric units such as `750 M.L.` and `750 M L`, compact estimated quantity marks such as `e750 mL`, comma thousands such as `1,000 mL`, whole or fractional pints, spelled-out number words such as `Seven Hundred Fifty Milliliters`, rounded dual-unit statements such as `750 mL / 25.4 FL OZ`, compound pint/ounce statements, and multipack statements such as `4 x 50 mL`, `4-50 mL`, `4-pack of 12 fl oz`, `4/12 fl oz cans`, or word-count `six 12 fl oz cans` are normalized to milliliters. Serving-size, recipe, calorie, or per-serving volumes are ignored so they do not create false net-contents mismatches.
