...
- Required brand materially mismatched. Brand matching is limited to primary label text and preserves word order. Harmless case, apostrophe, punctuation, ampersand/`AND`, `Co.`/`Company` legal-suffix, and common `St.`/`Saint` or `Mt.`/`Mount` variations pass, but a brand name appearing only in producer, importer, warning, or other non-brand context, or with reordered brand words, does not create a false pass.
- Product type materially mismatched. Explicit `DISTILLED SPIRITS`, distilled-spirits class/type terms such as `Gin` or `Whiskey`, `MALT BEVERAGE(S)`, beer-style terms, `MALT LIQUOR`, hard-cider/perry terms for wine-category products, and wine statements in product/class label context are prioritized over incidental descriptors or brand words such as `Wine Cask Finish` or `Spirit` inside a wine brand, even when the explicit product-type statement appears before the brand.
- Material ABV mismatch. Proof values from labels and formula support are converted to ABV, including low-proof values such as `40 Proof` = `20% ABV` and label wording such as `Proof 90` or `90 degrees proof`. Common label wording such as `Alc. 45% by Vol.`, `45% Alc. by Vol.`, `45% by volume`, `Alcohol 45% by Volume`, `Alcohol: 45% by Volume`, `13.5% vol`, and `Alcohol 45% by Vol.` is normalized to ABV. Non-alcohol percentages such as `100% Agave` are ignored for ABV/proof matching and become Needs Review if no actual alcohol statement is found.
- Material net contents mismatch. Milliliters, litres, British `millilitres`, decimal-comma litres such as `0,75 L`, centiliters, fluid ounces, plain ounces used as net contents, comma thousands such as `1,000 mL`, whole or fractional pints, spelled-out number words such as `Seven Hundred Fifty Milliliters`, rounded dual-unit statements such as `750 mL / 25.4 FL OZ`, and compound pint/ounce statements are normalized to milliliters. Serving-size, recipe, calorie, or per-serving volumes are ignored so they do not create false net-contents mismatches.
...
- Bottler/producer text appears only in brand or other non-responsible-party context instead of wording such as `Bottled by`, `Bottled for`, `Distilled by`, `Blended by`, `Imported by`, `Packed by`, or `Produced and bottled by`.
...
- Imported country of origin is missing or the country name appears only in unrelated text, such as an importer company name, rather than in origin wording like `Product of`, `Produce of`, `Distilled in`, `Distilled and bottled in`, `Blended in`, `Blended and bottled in`, `Brewed in`, `Brewed and bottled in`, `Made in`, `Imported from`, or `Imported by ... from`. United States and United Kingdom origins can match common label abbreviations such as `USA`, `U.S.A.`, `U.S.`, `UK`, or `U.K.`; United Kingdom can also match constituent-origin wording such as `Product of Scotland`, `Distilled in Scotland`, `Distilled and bottled in Scotland`, or `Blended in Scotland`; Ireland can match `Republic of Ireland`; and Netherlands can match `The Netherlands`.
