...
- Net contents and imported country checks are context-aware. Serving-size volumes are not treated as net contents, common multipack statements such as `4 x 50 mL` and `4-50 mL` are normalized to total volume, and a country name must appear in origin wording rather than only in unrelated importer or brand text.
