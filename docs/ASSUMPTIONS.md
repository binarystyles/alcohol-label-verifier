...
- Net contents and imported country checks are context-aware. Serving-size volumes are not treated as net contents, common multipack statements such as `4 x 50 mL`, `4-50 mL`, `4-pack of 12 fl oz`, `4/12 fl oz cans`, and word-count `six 12 fl oz cans` are normalized to total volume, and a country name must appear in origin wording rather than only in unrelated importer or brand text.
...
- Supplied Item 10 grape-varietal values and Item 11 wine-appellation values are treated as wine-only label-facing values when present. Missing or partially missing label evidence for those supplied values becomes Needs Review rather than being inferred from product type or label OCR.
