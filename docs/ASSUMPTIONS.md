...
- Each completed application PDF is self-contained and includes both application data and label artwork.
...
- Tesseract is installed locally in deployment environments through `packages.txt` or the Dockerfile.
- Filled TTB F 5100.31 PDFs may vary by producer, scanner, page size, and whether they contain AcroForm values or only raster imagery.
...
- Expected values such as class/type, ABV, and net contents may not be explicit on every blank form. The app supports an application-data summary block inside the same PDF for those values.
