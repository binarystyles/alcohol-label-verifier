...
- Item 9 on current TTB F 5100.31 is treated as a Formula ID/reference. The app looks for an exact normalized matching approved formula/source document inside the same uploaded application package and derives expected alcohol content from that matched document, not from label OCR and not from a runtime network lookup. Letter-prefixed and numeric Formula/TTB/Lab IDs are supported. Separator-only Formula ID differences are normalized, but prefix-sharing Formula IDs are not considered matches.
...
- Net contents and imported country checks are context-aware. Serving-size volumes are not treated as net contents, common multipack statements such as `4 x 50 mL`, `4-50 mL`, `4-pack of 12 fl oz`, and `4/12 fl oz cans` are normalized to total volume, and a country name must appear in origin wording rather than only in unrelated importer or brand text.
- Wine product-type checks recognize a conservative set of common grape-varietal type designations such as `Chardonnay`, `Cabernet Sauvignon`, `Pinot Noir`, and `Merlot`; unfamiliar varieties or unusual label phrasing may still require reviewer confirmation.
