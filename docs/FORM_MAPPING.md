...
Net contents checks normalize milliliters, liters, centiliters, fluid ounces, plain ounces used as net contents, comma thousands such as `1,000 mL`, whole or fractional pints, spelled-out number words such as `Seven Hundred Fifty Milliliters`, rounded dual-unit statements such as `750 mL / 25.4 FL OZ`, and compound pint/ounce statements, and ignore serving-size, recipe, calorie, and per-serving volumes so unrelated label text does not create a false mismatch. Imported country checks require origin-style label wording, such as `Product of Mexico`, `Produced in Mexico`, `Made in Mexico`, `Imported from Mexico`, `Imported by ... from Mexico`, or `Country of Origin Mexico`; a country name appearing only in an importer company name is not enough for a Pass. `United States` may also match common origin abbreviations such as `USA`, `U.S.A.`, and `U.S.`; `United Kingdom` may match `UK`, `U.K.`, `Great Britain`, or constituent-origin wording such as `Product of Scotland`; `Ireland` may match `Republic of Ireland`; and `Netherlands` may match `The Netherlands` when these variants appear in origin-style wording.
