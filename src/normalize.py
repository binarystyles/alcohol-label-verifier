...
def extract_product_type(text: str | None) -> str:
    ...
    if re.search(
        r"\b(?:BEER|ALE|LAGER|STOUT|PORTER|IPA|PILSNER|PILS|SAISON|GOSE|KOLSCH|KOELSCH|BOCK|DOPPELBOCK|WITBIER|HEFEWEIZEN|LAMBIC|CERVEZA|BARLEY\s*WINE|FMB|FLAVORED\s+MALT\s+BEVERAGE|HARD\s+SELTZER|SPIKED\s+SELTZER|MALT[-\s]+BASED[-\s]+SELTZER)\b",
        normalized,
    ) and not re.search(
    ...
    if re.search(r"\bWINE\b", normalized):
        return "WINE"
    if re.search(
        r"\b(?:SAKE|VERMOUTH|SHERRY|ANGELICA|MADEIRA|MUSCATEL|PORT|CHAMPAGNE|MEAD|HONEY\s+WINE|SANGRIA)\b",
        normalized,
    ):
        return "WINE"
    if re.search(
