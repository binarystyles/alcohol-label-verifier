...
def _is_explicit_product_type_line(line: str) -> bool:
    ...
    return bool(
        re.fullmatch(
            r"(?:PRODUCT\s+TYPE\s*:?\s*)?(?:DISTILLED\s+SPIRITS|MALT\s+BEVERAGES?|MALT\s+LIQUOR|BEER|ALE|LAGER|STOUT|PORTER|IPA|PILSNER|PILS|SAISON|GOSE|KOLSCH|KOELSCH|BOCK|DOPPELBOCK|WITBIER|HEFEWEIZEN|LAMBIC|CERVEZA|FMB|HARD\s+SELTZER|SPIKED\s+SELTZER|MALT[-\s]+BASED[-\s]+SELTZER|WINE|RED\s+WINE|WHITE\s+WINE|ROSE\s+WINE|SPARKLING\s+WINE|TABLE\s+WINE|DESSERT\s+WINE|HARD\s+CIDER|CIDER|PERRY)",
            normalized,
        )
    )
...
def _split_flat_label_line(line: str) -> list[str]:
    split_markers = (
        ...
        r"\b(?:BOTTLED|PRODUCED|DISTILLED|BLENDED|IMPORTED|BREWED|VINTED|CELLARED|CANNED|PACKED|PACKAGED|MANUFACTURED|DISTRIBUTED)(?:\s+AND\s+(?:BOTTLED|PRODUCED|DISTILLED|BLENDED|IMPORTED|BREWED|VINTED|CELLARED|CANNED|PACKED|PACKAGED|MANUFACTURED|DISTRIBUTED))*\s+(?:BY|FOR)\b",
...
def _is_obvious_non_class_line(line: str) -> bool:
    ...
    return bool(
        re.search(
            r"\b(DISTILLED\s+SPIRITS|MALT\s+BEVERAGES?|GOVERNMENT\s+WARNING|PRODUCT\s+OF|PRODUCED\s+IN|MADE\s+IN|COUNTRY\s+OF\s+ORIGIN|BOTTLED\s+BY|PRODUCED\s+BY|IMPORTED\s+BY|BREWED\s+BY|CANNED\s+BY|NET\s+CONTENTS?|SERVING\s+SIZE|ALC|ALCOHOL|PROOF)\b",
            normalized,
        )
    )
