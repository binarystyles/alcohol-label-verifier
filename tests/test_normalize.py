...
def test_abv_and_proof_extraction_normalize_to_abv() -> None:
    ...
    assert extract_abv_values("45% Alcohol/Vol.") == [45.0]
    assert extract_abv_values("45% Alcohol / Volume") == [45.0]
    assert extract_abv_values("Alcohol/Vol. 45%") == [45.0]
    assert extract_abv_values("Alcohol Volume 45%") == [45.0]
    ...
    assert extract_abv_values("Alcohol Volume 50 mL") == []
    assert extract_abv_values("Natural flavoring 49% by volume\n45% Alc./Vol.") == [45.0]
...
def test_net_contents_extraction_normalizes_units() -> None:
    ...
    assert extract_net_contents_values("Net Contents 4-pack 50 mL") == [200.0]
    assert extract_net_contents_values("Net Contents 4-pack of 12 fl oz cans") == [1419.528]
    assert extract_net_contents_values("Net Contents 4/12 fl oz cans") == [1419.528]
    assert extract_net_contents_values("Net Contents 6/355 mL bottles") == [2130.0]
